...
ACSELuaDatabase.AddContentToCall = function(_tContentToCall)
    ...
    -- Add per game LuaDatabase code, remove spaces from the names
    local all_trim = function(s)
       return string.gsub(s, "%s+", "")
    end
    local gamelua = "database." .. all_trim(api.game.GetGameName()) .. "luadatabase"
    if tryrequire(gamelua) then
        table.insert(_tContentToCall, require(gamelua))
    else
        api.debug.Trace("ACSE LuaDatabase for " .. api.game.GetGameName() .. " not found")
    end
...
ACSELuaDatabase.tDatabaseMethods = {
    ...
    AddLuaPrefab = function(_sName, _tParams)
        ...
        if type(_sName) == "string" and type(_tParams) == "table" then
            ...
            if api.acse.tLuaPrefabNames[_sName] == nil then
                -- Add new prefab
                table.insert(api.acse.tLuaPrefabs, {PrefabName = _sName, PrefabData = _tParams})
    ...
    Main.CallOnContent(
        ...
        function(_sName, _tParams)
            if type(_sName) == "string" and (type(_tParams) == "table" or type(_tParams) == 'function') then
                --api.debug.Trace("Accepting hook for " .. _sName)
                table.insert(
                    api.acse.tLuaHooks, 
                    {   
                        sName = _sName,
        ...
        function(_sName, _tParams)
            if type(_sName) == "string" and type(_tParams) == "table" then
                ...
                if api.game.GetGameName() == "Planet Coaster" then
                    sParkEnvironment = "Environments.CPTEnvironment"
                end
...
ACSELuaDatabase._Hook_ControlsOptionsMenu = function(tModule)
    ...
    tModule.GetItems = function(self, _tSettingsMenuItemsData, _sDeviceName)
        api.debug.Trace("AcseLuaDatabase:GetItems()")
        self:_ACSELuaDatabase_GetItems(_tSettingsMenuItemsData, _sDeviceName)
        for _, handler in global.ipairs(global.api.acse._tControlsSettingsRegistrations) do
            if handler.fGetItems then
                handler.fGetItems(_tSettingsMenuItemsData['items'], _sDeviceName)
            end
        end
    end
    tModule.HandleEvent = function(self, _sID, _arg)
            ...
            local fRebind = function(sControlName) 
                if api.game.GetGameName() == "Planet Zoo" or api.game.GetGameName() == "Planet Coaster" then
...
ACSELuaDatabase.tDefaultHooks = {
    ['Windows.KeyboardOptionsMenu'] = ACSELuaDatabase._Hook_ControlsOptionsMenu, -- JWE1/JWE2
    ['Windows.ControlsOptionsMenu'] = ACSELuaDatabase._Hook_ControlsOptionsMenu, -- PZ/PC
}
